...
for curr in range(4):
    ...
    toplot.append(toexport["DAC"][curr])
plt.plot([0,1,2,3],toplot,color=colors[0],marker="o",label="Ref. Sys.")

toplot=[]
for curr in range(4):
    #toplot.append(1-(toexport["LR_waystream"][curr]/toexport["LR"][curr]))
    toplot.append(toexport["LR"][curr])
plt.plot([0,1,2,3],toplot,color=colors[2],marker="o",label="Reg. LR")

toplot=[]
for curr in range(4):
    #toplot.append(1-(toexport["LR_waystream"][curr]/toexport["LR"][curr]))
    toplot.append(toexport["LR_waystream"][curr])
plt.plot([0,1,2,3],toplot,color=colors[4],marker="o",label="WS LR")

toplot=[]
for curr in range(4):
    #toplot.append(1-(toexport["BX_waystream"][curr]/toexport["BX"][curr]))
    toplot.append(toexport["BX"][curr])
plt.plot([0,1,2,3],toplot,color=colors[6],marker="o",label="Reg. BX")


toplot=[]
for curr in range(4):
    #toplot.append(1-(toexport["BX_waystream"][curr]/toexport["BX"][curr]))
    toplot.append(toexport["BX_waystream"][curr])
plt.plot([0,1,2,3],toplot,color=colors[8],marker="o",label="WS BX")



plt.xticks([0,1,2,3],configs)
plt.legend(loc="lower right",ncol=3,bbox_to_anchor=(1.1,1),fontsize=26)#, handleheight=0.9, labelspacing=0.2, handlelength=0.7,fontsize=13)#,
#plt.ylim(0,1)
...
#plt.show()
plt.savefig('ec_comparison_Switch.pdf')
plt.close()