...
plt.bar([0,2,4,6],[np.mean(x) for x in results],width=0.8,color=colors[3],label="Regular SFP")
plt.errorbar([0,2,4,6],[np.mean(x) for x in results],yerr=yerr,fmt='none',ecolor="black",elinewidth=20)
...
print(ci)
yerr = [np.array([np.mean(x) for x in results]) - np.array(ci[0]), np.array(ci[1]) - np.array([np.mean(x) for x in results])]
plt.bar([1,3,5,7],[np.mean(x) for x in results],width=0.8,color=colors[1],label="Waystream SFP")
plt.errorbar([1,3,5,7],[np.mean(x) for x in results],yerr=yerr,fmt='none',ecolor="black",elinewidth=20)
...
#plt.show()
plt.savefig('LR_power_comparison.png')
plt.close()
...
ax.grid(which='major', alpha=0.2)
#plt.show()
plt.savefig('BX_power_comparison.png')
plt.close()