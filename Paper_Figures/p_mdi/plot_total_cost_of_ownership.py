...
price_per_kwh=0.25 #https://ec.europa.eu/eurostat/statistics-explained/index.php?title=File:F6Electricity_prices_for_non-household_consumers,_second_half_2024_.png
gco2_per_kwh=207.1# https://www.eea.europa.eu/en/analysis/indicators/greenhouse-gas-emission-intensity-of-1
cost_LR_r = 38.08
cost_LR_w = cost_LR_r*1.25
cost_BX_r = 16.66
cost_BX_w = cost_BX_r*1.25
results=[]
baseline=[]
...
for curr in range(0,10*8760):
    kilo_watt_hour.append(([(np.mean(x)*curr/1000)*price_per_kwh+cost_LR_r for x in min_max]))
print("FS.com LR")
print((np.mean(min_max[1])/1000)*price_per_kwh*8760)
#print((np.mean(min_max[1])/1000)*gco2_per_kwh*8760*10)
...
min_max=[min(results),max(results)]
for curr in range(0,10*8760):
    kilo_watt_hour.append(([(np.mean(x)*curr/1000)*price_per_kwh+cost_LR_w for x in min_max]))
print("Waystream LR")
print((np.mean(min_max[1])/1000)*price_per_kwh*8760)
#print((np.mean(min_max[1])/1000)*gco2_per_kwh*8760*10)
...
min_max=[min(results),max(results)]
for curr in range(0,10*8760):
    kilo_watt_hour.append(([(np.mean(x)*curr/1000)*price_per_kwh+cost_BX_r for x in min_max]))
print("FS.com BX")
print((np.mean(min_max[1])/1000)*price_per_kwh*8760)
#print((np.mean(min_max[1])/1000)*gco2_per_kwh*8760*10)
...
min_max=[min(results),max(results)]
for curr in range(0,10*8760):
    kilo_watt_hour.append(([(np.mean(x)*curr/1000)*price_per_kwh+cost_BX_w for x in min_max]))
print("Waystream BX")
print((np.mean(min_max[1])/1000)*price_per_kwh*8760)
#print((np.mean(min_max[1])/1000)*gco2_per_kwh*8760*10)
...
#plt.show()
plt.savefig('total_cost_of_ownership.pdf')
plt.close()