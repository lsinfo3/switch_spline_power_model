        ...
        for curr in range(len(data["Voltage"])):
            if (data["Time"][curr] - data["Start_Time"][0]) > 130:
                ...
                current_results.append(
                    data["Real_Power"][curr])  # print(str(currfile)+": "+str(np.mean(current_results)))
    return all_results
...
for curr in range(4):
    ...
    raw_values_sfp=[]
    raw_values_dac=[]
    for currval in range(len(all_res["DAC"][curr])):
        try:
            raw_values.append(all_res["DAC"][curr][currval]/all_res["LR"][curr][currval])
            raw_values_sfp.append(all_res["LR"][curr][currval])
            raw_values_dac.append(all_res["DAC"][curr][currval])
        except:
...
plt.plot([0, 1, 2, 3], [np.mean(x)*100 for x in toplot], color=colors[0], marker="o", label="FS.com LR")
plt.annotate(
    text='FS.com LR',        # The text you want
    xy=(1, np.mean(toplot[1])*100),                # Point to annotate (arrow points here)
    xytext=(1, np.mean(toplot[1])*100+5),            # Position of the text
    arrowprops=dict(
...
for curr in range(len(toplot)):
    ...
    max_values.append(np.mean(toplot[curr]) * 100 + (math.sqrt(
        (math.sqrt(np.std(all_res_sfp[curr]) ** 2 + max_uncertainty) / np.mean(all_res_sfp[curr])) ** 2 + (
                math.sqrt(np.std(all_res_dac[curr]) ** 2 + max_uncertainty) / np.mean(
            all_res_dac[curr])) ** 2)) * np.mean(toplot[curr]) * 100)
    min_values.append(np.mean(toplot[curr]) * 100 - (math.sqrt(
        (math.sqrt(np.std(all_res_sfp[curr]) ** 2 + max_uncertainty) / np.mean(all_res_sfp[curr])) ** 2 + (
                math.sqrt(np.std(all_res_dac[curr]) ** 2 + max_uncertainty) / np.mean(
            all_res_dac[curr])) ** 2)) * np.mean(toplot[curr]) * 100)
plt.fill_between([ x for x in range(len(toplot))], max_values, min_values, color=colors[0], alpha=0.25)
...
for curr in range(4):
    ...
    for currval in range(len(all_res["DAC"][curr])):
        try:
            raw_values.append(all_res["DAC"][curr][currval]/all_res["LR_waystream"][curr][currval])
    ...
    all_res_sfp.append(raw_values_sfp)
    all_res_dac.append(raw_values_dac)
plt.plot([0, 1, 2, 3], [np.mean(x)*100 for x in toplot], color=colors[2], marker="o", label="Waystream LR")
plt.annotate(
    text='Waystream LR',        # The text you want
    xy=(0, np.mean(toplot[0])*100),                # Point to annotate (arrow points here)
    xytext=(0, np.mean(toplot[0])*100+5),            # Position of the text
    arrowprops=dict(
        arrowstyle='->',      # Arrow style
        color='red',          # Arrow color
        lw=2                  # Arrow line width
...
for curr in range(len(toplot)):
    ...
    min_values.append(np.mean(toplot[curr]) * 100 - (math.sqrt(
        (math.sqrt(np.std(all_res_sfp[curr]) ** 2 + max_uncertainty) / np.mean(all_res_sfp[curr])) ** 2 + (
                math.sqrt(np.std(all_res_dac[curr]) ** 2 + max_uncertainty) / np.mean(
            all_res_dac[curr])) ** 2)) * np.mean(toplot[curr]) * 100)
plt.fill_between([ x for x in range(len(toplot))], max_values, min_values, color=colors[2], alpha=0.25)
...
for curr in range(4):
    ...
    for currval in range(len(all_res["DAC"][curr])):
        try:
            raw_values.append(all_res["DAC"][curr][currval]/all_res["BX"][curr][currval])
    ...
    all_res_sfp.append(raw_values_sfp)
    all_res_dac.append(raw_values_dac)
plt.plot([0, 1, 2, 3], [np.mean(x)*100 for x in toplot], color=colors[4], marker="o", label="FS.com BX")
plt.annotate(
    text='FS.com BX',        # The text you want
    xy=(2, np.mean(toplot[2])*100),                # Point to annotate (arrow points here)
    xytext=(2, np.mean(toplot[2])*100+5),            # Position of the text
    arrowprops=dict(
        arrowstyle='->',      # Arrow style
        color='red',          # Arrow color
        lw=2                  # Arrow line width
...
for curr in range(len(toplot)):
    ...
    min_values.append(np.mean(toplot[curr]) * 100 - (math.sqrt(
        (math.sqrt(np.std(all_res_sfp[curr]) ** 2 + max_uncertainty) / np.mean(all_res_sfp[curr])) ** 2 + (
                math.sqrt(np.std(all_res_dac[curr]) ** 2 + max_uncertainty) / np.mean(
            all_res_dac[curr])) ** 2)) * np.mean(toplot[curr]) * 100)
plt.fill_between([ x for x in range(len(toplot))], max_values, min_values, color=colors[4], alpha=0.25)
...
for curr in range(4):
    ...
    for currval in range(len(all_res["DAC"][curr])):
        try:
            raw_values.append(all_res["DAC"][curr][currval]/all_res["BX_waystream"][curr][currval])
            raw_values_sfp.append(all_res["BX_waystream"][curr][currval])
            raw_values_dac.append(all_res["DAC"][curr][currval])
        except:
            pass # No more measurement points available
    toplot.append(raw_values)
    all_res_sfp.append(raw_values_sfp)
    all_res_dac.append(raw_values_dac)
plt.plot([0, 1, 2, 3], [np.mean(x)*100 for x in toplot], color=colors[6], marker="o", label="Waystream BX")
plt.annotate(
    text='Waystream BX',        # The text you want
    xy=(2, np.mean(toplot[2])*100),                # Point to annotate (arrow points here)
    xytext=(2, np.mean(toplot[2])*100-5),            # Position of the text
    arrowprops=dict(
        arrowstyle='->',      # Arrow style
...
for curr in range(len(toplot)):
    ...
    min_values.append(np.mean(toplot[curr]) * 100 - (math.sqrt(
        (math.sqrt(np.std(all_res_sfp[curr]) ** 2 + max_uncertainty) / np.mean(all_res_sfp[curr])) ** 2 + (
                math.sqrt(np.std(all_res_dac[curr]) ** 2 + max_uncertainty) / np.mean(
            all_res_dac[curr])) ** 2)) * np.mean(toplot[curr]) * 100)
plt.fill_between([ x for x in range(len(toplot))], max_values, min_values, color=colors[6], alpha=0.25)
...
#plt.show()
plt.savefig('ee_comparison_Switch.pdf')
plt.close()
